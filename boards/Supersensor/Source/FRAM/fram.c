...
int framWrite(uint8_t cmd, uint32_t *addr, uint8_t *src, uint32_t len) {
    ...
    if (addr != NULL) {
        ...
        // Need to set WEL bit for each WRSR/WRITE operation for Cypress's FRAM (Supersensor)
        // Need to set this only once for Fujitsu's FRAM with FTHR board and EV_Kit
        // Setting it here before every WRSR/WRITE operation because no harm when it's not required
        int retVal = framWrite(FRAM_SET_WRITE_EN_LATCH, NULL, NULL, 0);; 
        if (retVal != E_NO_ERROR) {
...
int initFRAMSPI() {
    ...
    spi_pins.ss0 = TRUE;    // FRAM
    spi_pins.ss1 = TRUE;    // FRAM
    spi_pins.ss2 = TRUE;    // voltage threshold resistor
    
    int retVal;
    retVal = MXC_SPI_Init(SPI, 1, 0, 3, 0, SPI_SPEED, spi_pins);
    if (retVal != E_NO_ERROR) {
